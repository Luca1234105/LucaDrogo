...
function Remove-MicrosoftApps {
    # Esempio: rimuove app Microsoft preinstallate
    Get-AppxPackage -AllUsers |
    Where-Object { $_.Name -notmatch "Store|Calculator" } |
    Remove-AppxPackage -ErrorAction SilentlyContinue
    [System.Windows.Forms.MessageBox]::Show("App Microsoft rimosse!")
}

function Remove-OneDrive {
    # Rimuove OneDrive completamente
    ...
    # Rimuovi residui (esempio)
    Remove-Item "$env:UserProfile\OneDrive" -Recurse -Force -ErrorAction SilentlyContinue
    [System.Windows.Forms.MessageBox]::Show("OneDrive rimosso!")
}

function Disable-Copilot {
    try {
        # Disabilita Copilot con regkey esempio (ipotetica)
        Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsCopilot" -Name "AllowCopilot" -Type DWord -Value 0 -Force
        [System.Windows.Forms.MessageBox]::Show("Copilot disabilitato!")
    } catch {
        [System.Windows.Forms.MessageBox]::Show("Errore disabilitazione Copilot: $_")
    }
}

function Remove-Widgets {
    # Rimuove widget di Windows 11 (esempio)
    Get-AppxPackage -AllUsers -Name "MicrosoftWindows.Client.WebExperience" | Remove-AppxPackage -ErrorAction SilentlyContinue
    [System.Windows.Forms.MessageBox]::Show("Widgets rimossi!")
}

function Remove-Edge {
    # Disinstalla Microsoft Edge (versione Chromium)
    $edgeSetup = "$env:ProgramFiles (x86)\Microsoft\Edge\Application\setup.exe"
    if (Test-Path $edgeSetup) {
        & $edgeSetup --uninstall --system-level --verbose-logging --force-uninstall
        [System.Windows.Forms.MessageBox]::Show("Edge rimosso!")
    } else {
        [System.Windows.Forms.MessageBox]::Show("Setup Edge non trovato.")
    }
}

function Invoke-DisattivaServizi {
    # Disattiva servizi superflui (esempio)
    $servicesToDisable = @(
        "DiagTrack",
        "WMPNetworkSvc",
        "XblGameSave",
    ...
    [System.Windows.Forms.MessageBox]::Show("Servizi disattivati!")
}
